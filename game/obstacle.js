...
// ground and obstacle should move at the same speed
const SPEED = 0.05;
// min. time to spawn/create an obstacle is 500 milliseconds
const OBSTACLE_INTERVAL_MIN = 500;
// max time to spawn/create an obstacle should be longer than 2000 milliseconds
...
export function updateObstacle(delta, speedScale) {
  // looping and querying over all obstacles
  document.querySelectorAll("[data-obstacle]").forEach((obstacle) => {
    incrementCustomProperty(
      obstacle,
      "--left",
    ...
    if (getCustomProperty(obstacle, "--left") <= -100) {
      // If these obstacles have a left property less than -100( defined in the css)
  ...
  if (nextObstacleTime <= 0) {
    createObstacle();
    // speed scale is here to spawn/create faster obstacle in the game to make it more difficult
    nextObstacleTime =
