...
import {
  ...
  getPlayerRect,
  setPlayerLose,
} from "./player.js"; // getPlayerRect() will get the dimensions of our player, use to check any collisions
...
const SPEED_SCALE_INCREASE = 0.00001; // difficulty increase as the game moves ahead

const gameElem = document.querySelector("[data-game]"); // main element that will put everything in here
...
setPixelToGameScale();
window.addEventListener("resize", setPixelToGameScale); // here whenever we resize the screen it will call `setPixelToGameScale()`
document.addEventListener("keydown", handleStart, { once: true }); // here whenever we press any key it will call `handleStart()`
...
// below function is for updating our player on each frame in a loop.
...
// below function is for adding various current scores which we got till the end of the gaming session.
function handleLose() {
